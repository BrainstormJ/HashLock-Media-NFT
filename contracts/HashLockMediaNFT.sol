...
/**
 * @title HashLockMediaNFT
 * @dev ERC-721 NFT contract for HashLock Media - tokenizing media licenses
 * @notice This contract allows minting of NFTs that represent licenses for media content
 *
 * Features:
 * - ERC-721 compliant NFT implementation
 * - URI storage for metadata (media details, license terms)
 * - Owner-controlled minting
 * - Automatic token ID management
 * - Burnable tokens
 * - Batch minting capabilities
 */
contract HashLockMediaNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    // Events
    event MediaLicenseMinted(
        address indexed recipient,
        uint256 indexed tokenId,
        string tokenURI
    );

    event MediaLicenseBurned(
    ...
    constructor(address initialOwner)
        ERC721("HashLock Media NFT", "HLMNFT")
    ...
    /**
     * @dev Safely mints a new media license NFT
    ...
    function safeMint(address to, string memory uri)
        ...
        returns (uint256)
    {
        require(to != address(0), "HashLockMediaNFT: mint to zero address");
        require(bytes(uri).length > 0, "HashLockMediaNFT: empty token URI");
        ...
        emit MediaLicenseMinted(to, tokenId, uri);

        return tokenId;
    }

    /**
     * @dev Burns (destroys) a media license NFT
    ...
    function burn(uint256 tokenId) public {
        require(
            ...
            "HashLockMediaNFT: caller is not owner nor approved"
        );

        address owner = ownerOf(tokenId);
        _burn(tokenId);

        emit MediaLicenseBurned(owner, tokenId);
    }
        ...
        returns (uint256[] memory)
    {
        require(
            ...
            "HashLockMediaNFT: arrays length mismatch"
        );
        require(
            recipients.length <= 100,
            "HashLockMediaNFT: batch size too large"
        );
    ...
    function updateTokenURI(uint256 tokenId, string memory uri)
        ...
        onlyOwner
    {
        require(_exists(tokenId), "HashLockMediaNFT: token does not exist");
        _setTokenURI(tokenId, uri);
    }
